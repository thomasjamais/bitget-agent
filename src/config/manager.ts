...
const configSchema = {
  type: 'object',
  required: ['api', 'instructions', 'globalRisk', 'marketData', 'ai', 'logging'],
  properties: {
    api: {
      type: 'object',
      required: ['key', 'secret', 'passphrase', 'useTestnet'],
      properties: {
    ...
    instructions: {
      ...
      items: {
        type: 'object',
        required: ['id', 'name', 'enabled', 'symbols', 'timeframes', 'direction', 'risk', 'signals'],
        properties: {
          ...
          risk: {
            type: 'object',
            required: ['maxRiskPerTrade', 'maxLeverage', 'maxPositionsPerSymbol', 'maxTotalPositions'],
            properties: {
    ...
    globalRisk: {
      type: 'object',
      required: ['maxEquityRisk', 'maxDailyLoss', 'maxConsecutiveLosses', 'sizingMethod'],
      properties: {
...
export class ConfigManager {
  ...
  getInstructionsForSymbol(symbol: string): InvestmentInstruction[] {
    ...
    return this.config.instructions.filter((instruction: InvestmentInstruction) => 
  ...
  private validateInstructions(instructions: InvestmentInstruction[]): void {
    ...
    for (const instruction of instructions) {
      ...
      if (risk.stopLossPercent && risk.takeProfitPercent) {
        if (risk.stopLossPercent >= risk.takeProfitPercent) {
          throw new Error(`Stop loss must be less than take profit for instruction: ${instruction.id}`);
        }
      }
      ...
      if (instruction.schedule?.startTime && instruction.schedule?.endTime) {
        ...
        if (start >= end) {
          throw new Error(`Start time must be before end time for instruction: ${instruction.id}`);
        }
      }
    }
  }
   ...
   */
  updateInstruction(instructionId: string, updates: Partial<InvestmentInstruction>): void {
    if (!this.config) {
      throw new Error('Configuration not loaded');
    }

    const instruction = this.config.instructions.find((i: InvestmentInstruction) => i.id === instructionId);
    if (!instruction) {
