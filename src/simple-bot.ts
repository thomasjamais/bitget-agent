 ...
 */
export class BitgetTradingBot {
  protected botState: BotState;
  protected riskManager: RiskManager;
  protected aiEngine: AIEngine | EnhancedAIEngine;
  protected decisionEngine: AggressiveDecisionEngine;
  protected portfolioBalancer: PortfolioBalancer;
  protected portfolioTransfer!: PortfolioTransfer;
  protected spotAutoBalancer!: SpotAutoBalancer;
  protected usdtTradingManager!: USDTTradingManager;
  protected wsAdapter: BotWebSocketAdapter;
  protected rest: any;
  protected ws: any;
  protected isRunning = false;
  protected readonly logger = logger.child({ component: "TradingBot" });
  protected runtimeConfig: { useTestnet?: boolean } = { useTestnet: false }; // Default to production
  ...
  private async continuouslyInvestFuturesBalance(): Promise<void> {
    try {
      ...
      if (bestOpportunity) {
        ...
        // Use 10% of the available balance for this trade
        const tradeAmount = availableUsdt * 0.1;
        const leverage = 5; // Use a moderate fixed leverage

        if (tradeAmount < 5) {
          this.logger.info(
            `💡 Trade amount is too small: ${tradeAmount.toFixed(
              2
            )} USDT < 5 USDT minimum`
          );
          return;
    ...
    try {
      ...
      for (const symbol of config.marketData.symbols) {
        ...
        if (bar) {
          ...
          this.logger.warn(
            `⚠️ Missing market data for ${symbol}, skipping rebalancing cycle.`
          );
          return;
        }
      }
      ...
      if (rebalanceActions.length > 0) {
        ...
        // Check if auto-rebalancing is enabled from dashboard
        const maxRebalanceTrades = 3; // Allow up to 3 rebalancing trades
   ...
   */
  protected async updateEquity(): Promise<void> {
    try {
      ...
      if (balance && balance.data) {
        ...
        if (balanceData.length > 0) {
          ...
          if (usdtBalance) {
            ...
            this.logger.error("❌ No USDT balance found in response");
            throw new Error("No USDT balance found in response");
          }
        } else {
          this.logger.error("❌ Empty balance data");
          throw new Error("Empty balance data");
        }
      } else {
        this.logger.error("❌ Invalid balance response");
   ...
   */
  protected async validateAPIConnection(): Promise<boolean> {
    try {
  ...
  private async updatePositions(): Promise<void> {
    try {
      ...
      if (positionsResponse && positionsResponse.data) {
        this.botState.positions = positionsResponse.data.map((p: any) => ({
          symbol: p.symbol,
          direction: p.holdSide,
          quantity: parseFloat(p.total),
          leverage: parseInt(p.leverage),
          orderType: "market", // Assuming market order for now
          price: parseFloat(p.averageOpenPrice),
        }));

        // Update daily PnL
        this.botState.dailyPnL = positionsResponse.data.reduce(
          (acc: number, p: any) => acc + parseFloat(p.unrealisedPnl || 0),
          0
        );
      }
    } catch (error) {
        ...
        dualPortfolio: dualPortfolioBalances
          ? {
              spot: {
                ...
                totalValue: await this.calculateSpotPortfolioValue(
                  dualPortfolioBalances.spot
                ),
              },
              futures: {
  ...
  async emergencyStop(): Promise<void> {
    ...
    try {
      ...
      if (positions && positions.data) {
        for (const position of positions.data) {
          const closeIntent: PositionIntent = {
            symbol: position.symbol,
            direction: position.holdSide === "long" ? "short" : "long",
            quantity: parseFloat(position.total), // Close the entire position
            leverage: parseInt(position.leverage),
            orderType: "market",
          };
          await this.executeTrade(closeIntent, "emergency-stop");
        }
      }
    } catch (error) {
      this.logger
        .error(`Failed to close all positions: $Failed to edit, 0 occurrences found for old_string (  async emergencyStop(): Promise<void> {
    this.logger.warn("Emergency stop initiated!");
    this.botState.isActive = false;

    // TODO: Close all open positions
    // This would typically involve getting all positions and placing opposite orders

    await this.stop();
  }). Original old_string was (  async emergencyStop(): Promise<void> {
    this.logger.warn("Emergency stop initiated!");
    this.botState.isActive = false;
    ...
    await this.stop();
  }) in /home/thomas/Dev/bitget-agent/src/simple-bot.ts. No edits made. The exact text in old_string was not found. Ensure you're not escaping content incorrectly and check whitespace, indentation, and context. Use read_file tool to verify.`);
    }
   ...
   */
  protected isTestnetMode(): boolean {
  ...
  private async getRecentTrades(): Promise<any[]> {
    try {
      // Try multiple API methods to get recent trades
      let trades: any[] = [];

      try {
        // Method 1: Get fills (executed trades)
        const fills = await this.rest.getFuturesFills({
          productType: "USDT-FUTURES",
        ...
        this.logger.debug(
          "getFuturesFills failed, trying alternative method:",
          fillsError
        );

        try {
          // Method 2: Get order history and filter for filled orders
          const orders = await this.rest.getFuturesOrders({
            productType: "USDT-FUTURES",
          ...
          trades =
            orders.data?.filter((order: any) => order.state === "filled") || [];
        } catch (ordersError) {
          this.logger.debug(
            "getFuturesOrders failed, trying positions method:",
            ordersError
          );

          try {
            // Method 3: Get current positions as a fallback
            const positions = await this.rest.getFuturesPositions({
              productType: "USDT-FUTURES",
            });

            // Convert positions to trade-like format
            trades =
              positions.data?.map((pos: any) => ({
                orderId: `pos_${pos.symbol}_${pos.holdSide}`,
                symbol: pos.symbol,
                side: pos.holdSide,
                size: pos.total,
            ...
            this.logger.debug(
              "All trade retrieval methods failed:",
              positionsError
            );
            return [];
          }
      ...
      return trades.map((trade: any) => ({
        ...
        symbol: trade.symbol,
        side: trade.side,
        amount: parseFloat(trade.size || trade.total || "0"),
        price: parseFloat(trade.price || trade.averagePrice || "0"),
        timestamp: trade.cTime,
        status: trade.state === "filled" ? "filled" : "pending",
        pnl: parseFloat(trade.pnl || trade.unrealizedPnl || "0"),
      }));
    } catch (error) {
  ...
  /**
   * Calculate spot portfolio value using real-time prices
   */
  private async calculateSpotPortfolioValue(
    spotBalances: any
  ): Promise<number> {
    try {
      let totalValue = spotBalances.USDT || 0;

      // Debug: Log spot balances
      this.logger.info("🔍 Spot balances for calculation:", {
        USDT: spotBalances.USDT || 0,
        BTC: spotBalances.BTC || 0,
        ETH: spotBalances.ETH || 0,
        BNB: spotBalances.BNB || 0,
        MATIC: spotBalances.MATIC || 0,
        raw: spotBalances,
      });

      // Get current market prices (with fallback to API)
      let btcPrice = this.marketData.get("BTCUSDT")?.close || 0;
      let ethPrice = this.marketData.get("ETHUSDT")?.close || 0;
      let bnbPrice = this.marketData.get("BNBUSDT")?.close || 0;
      let maticPrice = this.marketData.get("MATICUSDT")?.close || 0;

      // If market data is not available, fetch prices directly from API
      if (btcPrice === 0 || ethPrice === 0 || bnbPrice === 0) {
        try {
          this.logger.info(
            "🔍 Market data not available, fetching prices from API..."
          );

          const [btcTicker, ethTicker, bnbTicker] = await Promise.all([
            this.rest.getFuturesTicker({
              productType: "USDT-FUTURES",
              symbol: "BTCUSDT",
            }),
            this.rest.getFuturesTicker({
          ...
          btcPrice = parseFloat(btcTicker.data[0]?.lastPr || "0");
          ethPrice = parseFloat(ethTicker.data[0]?.lastPr || "0");
          bnbPrice = parseFloat(bnbTicker.data[0]?.lastPr || "0");

          this.logger.info("🔍 Fetched prices from API:", {
            btcPrice,
            ethPrice,
            bnbPrice,
          });
        } catch (error) {
          this.logger.warn("Failed to fetch market prices from API:", error);
        }
      }

      // Debug: Log market prices
      this.logger.info("🔍 Market prices for calculation:", {
        BTCUSDT: btcPrice,
        ETHUSDT: ethPrice,
        BNBUSDT: bnbPrice,
        MATICUSDT: maticPrice,
      });

      // Calculate values using real-time prices
      if (btcPrice > 0) totalValue += (spotBalances.BTC || 0) * btcPrice;
      if (ethPrice > 0) totalValue += (spotBalances.ETH || 0) * ethPrice;
      if (bnbPrice > 0) totalValue += (spotBalances.BNB || 0) * bnbPrice;
      if (maticPrice > 0) totalValue += (spotBalances.MATIC || 0) * maticPrice;

      this.logger.info("🔍 Calculated total spot value:", totalValue);

      // If calculation failed, use a simple fallback
      if (totalValue === 0 && spotBalances.USDT > 0) {
        this.logger.warn(
          "⚠️ Portfolio calculation failed, using USDT only fallback"
        );
        return spotBalances.USDT;
      }

      return totalValue;
    } catch (error) {
      this.logger.error("Failed to calculate spot portfolio value:", error);
      return spotBalances.USDT || 0; // Fallback to USDT only
    }
  }
  ...
  /**
   * Update risk strategy for USDT trading
   */
  updateRiskStrategy(strategy: "moderate" | "intense" | "risky"): void {
    if (this.usdtTradingManager) {
      this.usdtTradingManager.updateRiskStrategy(strategy);
    }
  }

  /**
   * Update bot configuration from dashboard
   */
  updateBotConfiguration(config: any): void {
    this.logger.info("⚙️ Bot configuration updated from dashboard:", config);

    // Update USDT trading manager configuration if available
    if (this.usdtTradingManager && config.maxRiskPerTrade) {
      this.usdtTradingManager.updateConfig({
        riskManagement: {
          maxRiskPerTrade: config.maxRiskPerTrade,
          stopLossPercentage: config.stopLoss || 2.5,
          takeProfitPercentage: config.takeProfit || 5.0,
        },
      });
    }
